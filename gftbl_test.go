...
func TestInverseTbl(t *testing.T) {
	for i := 0; i <= 255; i++ {
		a := inverseTbl[i]
		if i == 0 {
			if a != 0 {
				t.Fatalf("0's inverse should be 0, but got %d", a)
			}
			continue
		}
		v := mulTbl[a][i]
		if v != 1 {
			t.Fatalf("%d * its inverse should be 1, but got: %d", i, v)
		}
	}
}

func TestLowHighTbl(t *testing.T) {
	for i := 0; i <= 255; i++ {
		tbl := lowHighTbl[i*32 : i*32+32]
		lowTbl := tbl[:16]
		highTbl := tbl[16:32]
		for j := 0; j <= 255; j++ {
			exp := mulTbl[i][j]
